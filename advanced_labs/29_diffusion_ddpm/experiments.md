# Plan de experimentos — Difusión DDPM sobre Fashion-MNIST

<!-- nav-top -->
> 🧭 **Clase 30 / 31** · 🔬 [Módulo 7 — Especializaciones avanzadas](../../parts/07-especializaciones-avanzadas.md)
>
> [⬅️ 🖌️ WGAN-GP sobre Fashion-MNIST](../../advanced_labs/28_wgan_gp/experiments.md) · [🏠 Índice de clases](../../parts/README.md) · [🪞 Aprendizaje autosupervisado SimCLR ➡️](../../advanced_labs/30_self_supervised_simclr/experiments.md)
>
...
Formulada como algo que se puede refutar: **el modelo de este laboratorio supera a Autoencoder generativo simple en `noise_mse`, y la diferencia es mayor que la dispersión entre semillas.**

Qué la haría falsa —y esto también es un resultado que hay que reportar—:

- El modelo no supera a Autoencoder generativo simple.
...
Además de la comparación con la línea base, aquí interesa una pregunta específica: **Comparar 100 y 500 pasos, cronograma lineal y coseno, con igual dataset**. Es la comparación que da sentido al tema de este laboratorio; la de la línea base solo dice si el modelo sirve, mientras que esta dice *qué parte* del diseño es la que aporta.
...
| El conjunto `test` | **Intacto** hasta el final | Se abre una sola vez, después de que `noise_mse` sobre `validation` haya elegido al ganador. |
...
```bash
neural-labs train-advanced --track 29_diffusion_ddpm --split-seed 42 --training-seed 41
neural-labs train-advanced --track 29_diffusion_ddpm --split-seed 42 --training-seed 42
...
- **Métrica en `validation`**: el mejor valor de `noise_mse` durante el entrenamiento; está en `history.csv` y resumido en `metrics.json`.
...
5. **Vuelve a la línea base.** La comparación contra Autoencoder generativo simple es la que dice si todo el aparato de la red neuronal estaba justificado para este problema.
