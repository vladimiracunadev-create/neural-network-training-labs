# Plan de experimentos — Fine-tuning eficiente de transformer

<!-- nav-top -->
> 🧭 **Clase 26 / 31** · 🔬 [Módulo 7 — Especializaciones avanzadas](../../parts/07-especializaciones-avanzadas.md)
>
> [⬅️ 🏁 Proyecto final: churn de telecomunicaciones](../../labs/24_capstone_real_project/experiments.md) · [🏠 Índice de clases](../../parts/README.md) · [🧷 Segmentación semántica con U-Net ➡️](../../advanced_labs/26_segmentation_unet/experiments.md)
>
...
Formulada como algo que se puede refutar: **el modelo de este laboratorio supera a TF-IDF + regresión logística en `accuracy`, y la diferencia es mayor que la dispersión entre semillas.**

Qué la haría falsa —y esto también es un resultado que hay que reportar—:

- El modelo no supera a TF-IDF + regresión logística.
...
Además de la comparación con la línea base, aquí interesa una pregunta específica: **Comparar fine-tuning completo con LoRA manteniendo la partición y la semilla de entrenamiento**. Es la comparación que da sentido al tema de este laboratorio; la de la línea base solo dice si el modelo sirve, mientras que esta dice *qué parte* del diseño es la que aporta.
...
| El conjunto `test` | **Intacto** hasta el final | Se abre una sola vez, después de que `accuracy` sobre `validation` haya elegido al ganador. |
...
```bash
neural-labs train-advanced --track 25_transformer_finetuning --split-seed 42 --training-seed 41
neural-labs train-advanced --track 25_transformer_finetuning --split-seed 42 --training-seed 42
...
- **Métrica en `validation`**: el mejor valor de `accuracy` durante el entrenamiento; está en `history.csv` y resumido en `metrics.json`.
...
5. **Vuelve a la línea base.** La comparación contra TF-IDF + regresión logística es la que dice si todo el aparato de la red neuronal estaba justificado para este problema.
