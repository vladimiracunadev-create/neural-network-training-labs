# Plan de experimentos — Aprendizaje autosupervisado SimCLR

<!-- nav-top -->
> 🧭 **Clase 31 / 31** · 🔬 [Módulo 7 — Especializaciones avanzadas](../../parts/07-especializaciones-avanzadas.md)
>
> [⬅️ 🌫️ Difusión DDPM sobre Fashion-MNIST](../../advanced_labs/29_diffusion_ddpm/experiments.md) · [🏠 Índice de clases](../../parts/README.md) · *fin del recorrido* ➡️
>
...
Formulada como algo que se puede refutar: **el modelo de este laboratorio supera a ResNet18 aleatoria + linear probe en `nt_xent`, y la diferencia es mayor que la dispersión entre semillas.**

Qué la haría falsa —y esto también es un resultado que hay que reportar—:

- El modelo no supera a ResNet18 aleatoria + linear probe.
...
Además de la comparación con la línea base, aquí interesa una pregunta específica: **Comparar intensidad de aumento y tamaño de batch manteniendo fijo el split**. Es la comparación que da sentido al tema de este laboratorio; la de la línea base solo dice si el modelo sirve, mientras que esta dice *qué parte* del diseño es la que aporta.
...
| El conjunto `test` | **Intacto** hasta el final | Se abre una sola vez, después de que `nt_xent` sobre `validation` haya elegido al ganador. |
...
```bash
neural-labs train-advanced --track 30_self_supervised_simclr --split-seed 42 --training-seed 41
neural-labs train-advanced --track 30_self_supervised_simclr --split-seed 42 --training-seed 42
...
- **Métrica en `validation`**: el mejor valor de `nt_xent` durante el entrenamiento; está en `history.csv` y resumido en `metrics.json`.
...
5. **Vuelve a la línea base.** La comparación contra ResNet18 aleatoria + linear probe es la que dice si todo el aparato de la red neuronal estaba justificado para este problema.
