# Plan de experimentos — WGAN-GP sobre Fashion-MNIST

<!-- nav-top -->
> 🧭 **Clase 29 / 31** · 🔬 [Módulo 7 — Especializaciones avanzadas](../../parts/07-especializaciones-avanzadas.md)
>
> [⬅️ 🎙️ Clasificación de audio con SpeechCommands](../../advanced_labs/27_audio_speechcommands/experiments.md) · [🏠 Índice de clases](../../parts/README.md) · [🌫️ Difusión DDPM sobre Fashion-MNIST ➡️](../../advanced_labs/29_diffusion_ddpm/experiments.md)
>
...
Formulada como algo que se puede refutar: **el modelo de este laboratorio supera a DCGAN convolucional en `wasserstein_estimate`, y la diferencia es mayor que la dispersión entre semillas.**

Qué la haría falsa —y esto también es un resultado que hay que reportar—:

- El modelo no supera a DCGAN convolucional.
...
Además de la comparación con la línea base, aquí interesa una pregunta específica: **Comparar DCGAN y WGAN-GP con igual presupuesto de actualizaciones**. Es la comparación que da sentido al tema de este laboratorio; la de la línea base solo dice si el modelo sirve, mientras que esta dice *qué parte* del diseño es la que aporta.
...
| El conjunto `test` | **Intacto** hasta el final | Se abre una sola vez, después de que `wasserstein_estimate` sobre `validation` haya elegido al ganador. |
...
```bash
neural-labs train-advanced --track 28_wgan_gp --split-seed 42 --training-seed 41
neural-labs train-advanced --track 28_wgan_gp --split-seed 42 --training-seed 42
...
- **Métrica en `validation`**: el mejor valor de `wasserstein_estimate` durante el entrenamiento; está en `history.csv` y resumido en `metrics.json`.
...
5. **Vuelve a la línea base.** La comparación contra DCGAN convolucional es la que dice si todo el aparato de la red neuronal estaba justificado para este problema.
