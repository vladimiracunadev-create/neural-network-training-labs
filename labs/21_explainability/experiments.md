...
>
> [⬅️ 🔄 Aumento de datos](../../labs/20_data_augmentation/experiments.md) · [🏠 Índice de clases](../../parts/README.md) · [🎯 Incertidumbre y calibración ➡️](../../labs/22_uncertainty_calibration/experiments.md)
>
...
Formulada como algo que se puede refutar: **el modelo de este laboratorio supera a Regresión logística interpretable en `f1`, y la diferencia es mayor que la dispersión entre semillas.**

Qué la haría falsa —y esto también es un resultado que hay que reportar—:

- El modelo no supera a Regresión logística interpretable.
...
Además de la comparación con la línea base, aquí interesa una pregunta específica: **Contrastar explicaciones con permutaciones**. Es la comparación que da sentido al tema de este laboratorio; la de la línea base solo dice si el modelo sirve, mientras que esta dice *qué parte* del diseño es la que aporta.
...
| El conjunto `test` | **Intacto** hasta el final | Se abre una sola vez, después de que `f1` sobre `validation` haya elegido al ganador. |
...
```bash
neural-labs benchmark --lab 21_explainability \
...
- **Métrica en `validation`**: el mejor valor de `f1` durante el entrenamiento; está en `history.csv` y resumido en `metrics.json`.
...
5. **Vuelve a la línea base.** La comparación contra Regresión logística interpretable es la que dice si todo el aparato de la red neuronal estaba justificado para este problema.
