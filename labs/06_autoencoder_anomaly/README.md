...
>
> [⬅️ 📈 LSTM para series temporales](../../labs/05_lstm_time_series/README.md) · [🏠 Índice de clases](../../parts/README.md) · [🔭 Transformer para noticias ➡️](../../labs/07_transformer_attention/README.md)
>
...
> **Pregunta esencial:** ¿Cuándo un error de reconstrucción se convierte en evidencia de anomalía?
...
*Mapa de esta clase: Del patrón normal al umbral de alerta. La figura no es decorativa; úsala para explicar el mecanismo con tus propias palabras.*

## 🎯 Qué vas a aprender y construir

Detectar transacciones fraudulentas mediante error de reconstrucción.

**Práctica propia de esta clase:** Comparar reconstrucciones, estudiar las distribuciones de error y elegir el umbral únicamente con validation.

Es la **clase 07 de 31** del programa y pertenece a 🔵 el módulo 2, *Arquitecturas según la forma del dato*. Llegas desde **LSTM para series temporales** y lo que hagas aquí lo da por supuesto **Transformer para noticias**.
...
> Un error alto no prueba fraude; señala una observación distinta de las aprendidas y necesita interpretación contextual.
...
La clase reporta `precision`, `recall`, `f1`, `roc_auc`, `pr_auc`. De todas ellas, la que **decide** qué modelo se conserva es `f1`, y se mide siempre sobre `validation`: es la única forma de que `test` siga siendo una estimación honesta de lo que pasará con datos nuevos.
