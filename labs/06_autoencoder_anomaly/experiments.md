...
Formulada como algo que se puede refutar: **el modelo de este laboratorio supera a Isolation Forest en `f1`, y la diferencia es mayor que la dispersión entre semillas.**

Qué la haría falsa —y esto también es un resultado que hay que reportar—:

- El modelo no supera a Isolation Forest.
...
```bash
neural-labs benchmark --lab 06_autoencoder_anomaly \
...
5. **Vuelve a la línea base.** La comparación contra Isolation Forest es la que dice si todo el aparato de la red neuronal estaba justificado para este problema.
