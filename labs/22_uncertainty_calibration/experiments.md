...
Formulada como algo que se puede refutar: **el modelo de este laboratorio supera a Regresión logística calibrada en `f1`, y la diferencia es mayor que la dispersión entre semillas.**

Qué la haría falsa —y esto también es un resultado que hay que reportar—:

- El modelo no supera a Regresión logística calibrada.
...
```bash
neural-labs benchmark --lab 22_uncertainty_calibration \
...
5. **Vuelve a la línea base.** La comparación contra Regresión logística calibrada es la que dice si todo el aparato de la red neuronal estaba justificado para este problema.
