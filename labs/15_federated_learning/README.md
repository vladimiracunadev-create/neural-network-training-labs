...
Es la **ruta 16 de 31** y pertenece a 🟠 [la parte 4, Entrenar mejor, más barato y sin centralizar datos](../../parts/04-entrenamiento-eficiente.md). Llegas desde [⚗️ Destilación de conocimiento](../../labs/14_knowledge_distillation/README.md) y lo que aprendas aquí lo da por supuesto [∂ Backpropagation manual](../../labs/16_backpropagation_manual/README.md).

## 🧠 La idea que se pone a prueba

Este laboratorio trabaja **w_{t+1}=Σ_k(n_k/n)w_k**.
...
> **La pregunta que deberías poder responder al final:** ¿Qué clientes quedan perjudicados por la agregación?

**Métricas que se reportan:** `accuracy`, `macro_f1`, `client_accuracy_std`. La selección del modelo se decide con `macro_f1` sobre `validation`.
