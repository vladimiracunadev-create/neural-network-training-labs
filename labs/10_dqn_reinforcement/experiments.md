...
>
> [⬅️ 🕸️ GNN sobre red de citas](../../labs/09_gnn_graphs/experiments.md) · [🏠 Índice de clases](../../parts/README.md) · [♻️ Transfer learning con mascotas ➡️](../../labs/11_transfer_learning/experiments.md)
>
...
Formulada como algo que se puede refutar: **el modelo de este laboratorio supera a Política de reposición periódica basada en demanda media histórica en `mean_return`, y la diferencia es mayor que la dispersión entre semillas.**

Qué la haría falsa —y esto también es un resultado que hay que reportar—:

- El modelo no supera a Política de reposición periódica basada en demanda media histórica.
...
Además de la comparación con la línea base, aquí interesa una pregunta específica: **Comparar retorno contra políticas heurísticas**. Es la comparación que da sentido al tema de este laboratorio; la de la línea base solo dice si el modelo sirve, mientras que esta dice *qué parte* del diseño es la que aporta.
...
| El conjunto `test` | **Intacto** hasta el final | Se abre una sola vez, después de que `mean_return` sobre `validation` haya elegido al ganador. |
...
```bash
neural-labs benchmark --lab 10_dqn_reinforcement \
...
- **Métrica en `validation`**: el mejor valor de `mean_return` durante el entrenamiento; está en `history.csv` y resumido en `metrics.json`.
...
5. **Vuelve a la línea base.** La comparación contra Política de reposición periódica basada en demanda media histórica es la que dice si todo el aparato de la red neuronal estaba justificado para este problema.
