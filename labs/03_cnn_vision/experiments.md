...
>
> [⬅️ 🌀 MLP multiclase](../../labs/02_mlp_nonlinear/experiments.md) · [🏠 Índice de clases](../../parts/README.md) · [🔁 RNN para texto ➡️](../../labs/04_rnn_sequences/experiments.md)
>
...
Formulada como algo que se puede refutar: **el modelo de este laboratorio supera a Clasificador lineal sobre píxeles en `macro_f1`, y la diferencia es mayor que la dispersión entre semillas.**

Qué la haría falsa —y esto también es un resultado que hay que reportar—:

- El modelo no supera a Clasificador lineal sobre píxeles.
...
Además de la comparación con la línea base, aquí interesa una pregunta específica: **Comparar kernel, normalización y aumento**. Es la comparación que da sentido al tema de este laboratorio; la de la línea base solo dice si el modelo sirve, mientras que esta dice *qué parte* del diseño es la que aporta.
...
```bash
neural-labs benchmark --lab 03_cnn_vision \
...
5. **Vuelve a la línea base.** La comparación contra Clasificador lineal sobre píxeles es la que dice si todo el aparato de la red neuronal estaba justificado para este problema.
