...
>
> [⬅️ 🔁 RNN para texto](../../labs/04_rnn_sequences/experiments.md) · [🏠 Índice de clases](../../parts/README.md) · [🧬 Autoencoder para fraude ➡️](../../labs/06_autoencoder_anomaly/experiments.md)
>
...
Formulada como algo que se puede refutar: **el modelo de este laboratorio supera a Persistencia, media móvil y Ridge en `rmse`, y la diferencia es mayor que la dispersión entre semillas.**

Qué la haría falsa —y esto también es un resultado que hay que reportar—:

- El modelo no supera a Persistencia, media móvil y Ridge.
...
Además de la comparación con la línea base, aquí interesa una pregunta específica: **Comparar ventanas, variables y horizonte**. Es la comparación que da sentido al tema de este laboratorio; la de la línea base solo dice si el modelo sirve, mientras que esta dice *qué parte* del diseño es la que aporta.
...
| El conjunto `test` | **Intacto** hasta el final | Se abre una sola vez, después de que `rmse` sobre `validation` haya elegido al ganador. |
...
```bash
neural-labs benchmark --lab 05_lstm_time_series \
...
- **Métrica en `validation`**: el mejor valor de `rmse` durante el entrenamiento; está en `history.csv` y resumido en `metrics.json`.
...
5. **Vuelve a la línea base.** La comparación contra Persistencia, media móvil y Ridge es la que dice si todo el aparato de la red neuronal estaba justificado para este problema.
